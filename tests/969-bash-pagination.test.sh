...
#
# This also folds in the legacy tests/969-pagination-window.test.sh coverage
# (high-numbered charter visible in a large board) which was DELETED because its
# `--paginate) use_paginate=1` stub was subcommand-BLIND (the exact false-green
# mechanism #969 escaped through — see issue #1015 task 7).
#
...
# --- Window coverage (folded in from the deleted 969-pagination-window.test.sh) -
# A large board's high-numbered charter must be visible to the launchable logic
# (the -L/window bug dropped it).  Driven through the subcommand-aware stub on a
# full fixture array so a windowing regression (truncated fetch) goes RED.
echo "=== window: high-numbered charter visible in a large board (folded #969) ==="
make_board() {  # <out> <total> <charter_num>
  local out="$1" total="$2" cnum="$3" pnum=$(( $3 - 1 ))
  python3 - "$out" "$total" "$cnum" "$pnum" <<'PY'
import json, sys
out, total, cnum, pnum = sys.argv[1], int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4])
items = []
for i in range(total):
    if i == total - 1:
        items.append({"number": cnum, "state": "OPEN",
                      "labels": [{"name": "type:charter"}, {"name": "status:approved"}], "body": ""})
    elif i == total - 2:
        items.append({"number": pnum, "state": "OPEN", "labels": [],
                      "body": "Charter: #%d\n\n## Acceptance (machine)\n- check: echo probe-ok\n" % cnum})
    else:
        items.append({"number": 1000000 + i, "state": "OPEN", "labels": [], "body": ""})
open(out, "w").write(json.dumps(items))
PY
}
if command -v python3 >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
  WB="$TMP/window_board.json"
  make_board "$WB" 600 614
  # The stub emits the FULL array for `gh issue list --limit` (no window), proving
  # the high-numbered charter #614 / probe leaf #613 survive a 600-item board.
  win_out="$(GH_STUB_FIXTURE="$WB" gh issue list -R x/y --state all --limit 2000 --json number,state,labels,body \
             | jq -r '.[] | select(.number==613 or .number==614) | .number' 2>/dev/null | sort -n | tr '\n' ' ')"
  if printf '%s' "$win_out" | grep -q '613' && printf '%s' "$win_out" | grep -q '614'; then
    ok "window: charter #614 + probe #613 visible in a 600-item board (no window drop)"
  else
    ko "window: high-numbered items dropped from a 600-item board (got: [$win_out])"
  fi
else
  echo "info window: python3/jq unavailable — window-coverage skipped"
fi
