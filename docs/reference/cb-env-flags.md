...
| `CB_PLAN_CONVERGE_CAP` | *(forthcoming)* | launcher | Planned companion to `CB_CONVERGE_CAP` for the plan-review loop — forthcoming (see §Forthcoming below). |
...
**`CB_PLAN_CONVERGE_CAP`** is not yet implemented in the launcher source.  It is
planned as a companion to `CB_CONVERGE_CAP` that will cap the number of tech-lead
planning rounds in the plan-review loop independently of the analyst ↔ reviewer
substance-convergence limit.  Once implemented it will accept the same integer values
as `CB_CONVERGE_CAP` and will default to `CB_CONVERGE_CAP` when unset.
