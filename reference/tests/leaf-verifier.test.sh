...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=19 EXCLUDED=49) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_excl_count" -eq 49 ] \
    && ok "guard: EXCLUDED count=49" \
    || ko "guard: EXCLUDED count expected 49, got $_excl_count"

  [ "$_actual_count" -eq 68 ] \
    && ok "guard: actual *.test.sh count=68" \
