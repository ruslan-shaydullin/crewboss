...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=15 EXCLUDED=39) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_allow_count" -eq 15 ] \
    && ok "guard: ALLOW count=15" \
    || ko "guard: ALLOW count expected 15, got $_allow_count"

  [ "$_excl_count" -eq 39 ] \
    && ok "guard: EXCLUDED count=39" \
    || ko "guard: EXCLUDED count expected 39, got $_excl_count"

  [ "$_actual_count" -eq 54 ] \
    && ok "guard: actual *.test.sh count=54" \
    || ko "guard: actual *.test.sh count expected 54, got $_actual_count"
