...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=18 EXCLUDED=48) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_actual_count" -eq 66 ] \
    && ok "guard: actual *.test.sh count=66" \
    || ko "guard: actual *.test.sh count expected 66, got $_actual_count"
