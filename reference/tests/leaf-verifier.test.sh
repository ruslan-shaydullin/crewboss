...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=17 EXCLUDED=44) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_excl_count" -eq 44 ] \
    && ok "guard: EXCLUDED count=44" \
    || ko "guard: EXCLUDED count expected 44, got $_excl_count"

  [ "$_actual_count" -eq 61 ] \
    && ok "guard: actual *.test.sh count=61" \
    || ko "guard: actual *.test.sh count expected 61, got $_actual_count"
