...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=17 EXCLUDED=45) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_actual_count" -eq 62 ] \
    && ok "guard: actual *.test.sh count=62" \
    || ko "guard: actual *.test.sh count expected 62, got $_actual_count"
