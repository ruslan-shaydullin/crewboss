...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=11 EXCLUDED=36) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_actual_count" -eq 47 ] \
    && ok "guard: actual *.test.sh count=47" \
    || ko "guard: actual *.test.sh count expected 47, got $_actual_count"
