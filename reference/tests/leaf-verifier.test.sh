...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=17 EXCLUDED=42) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_excl_count" -eq 42 ] \
    && ok "guard: EXCLUDED count=42" \
    || ko "guard: EXCLUDED count expected 42, got $_excl_count"

  [ "$_actual_count" -eq 59 ] \
    && ok "guard: actual *.test.sh count=59" \
    || ko "guard: actual *.test.sh count expected 59, got $_actual_count"
