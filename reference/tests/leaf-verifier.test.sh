...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=18 EXCLUDED=45) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_allow_count" -eq 18 ] \
    && ok "guard: ALLOW count=18" \
    || ko "guard: ALLOW count expected 18, got $_allow_count"

  [ "$_excl_count" -eq 45 ] \
    && ok "guard: EXCLUDED count=45" \
    || ko "guard: EXCLUDED count expected 45, got $_excl_count"

  [ "$_actual_count" -eq 63 ] \
    && ok "guard: actual *.test.sh count=63" \
    || ko "guard: actual *.test.sh count expected 63, got $_actual_count"
