...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=20 EXCLUDED=49) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_actual_count" -eq 69 ] \
    && ok "guard: actual *.test.sh count=69" \
    || ko "guard: actual *.test.sh count expected 69, got $_actual_count"
