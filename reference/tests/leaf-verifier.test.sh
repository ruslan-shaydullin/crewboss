...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=17 EXCLUDED=43) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_excl_count" -eq 43 ] \
    && ok "guard: EXCLUDED count=43" \
    || ko "guard: EXCLUDED count expected 43, got $_excl_count"

  [ "$_actual_count" -eq 60 ] \
    && ok "guard: actual *.test.sh count=60" \
    || ko "guard: actual *.test.sh count expected 60, got $_actual_count"
