...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=18 EXCLUDED=47) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_excl_count" -eq 47 ] \
    && ok "guard: EXCLUDED count=47" \
    || ko "guard: EXCLUDED count expected 47, got $_excl_count"

  [ "$_actual_count" -eq 65 ] \
    && ok "guard: actual *.test.sh count=65" \
    || ko "guard: actual *.test.sh count expected 65, got $_actual_count"
