...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=18 EXCLUDED=46) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_excl_count" -eq 46 ] \
    && ok "guard: EXCLUDED count=46" \
    || ko "guard: EXCLUDED count expected 46, got $_excl_count"

  [ "$_actual_count" -eq 64 ] \
    && ok "guard: actual *.test.sh count=64" \
    || ko "guard: actual *.test.sh count expected 64, got $_actual_count"
