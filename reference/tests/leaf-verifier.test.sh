...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=22 EXCLUDED=58) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ $_excl_count -eq 58 ] \
    ...
    || ko "guard: EXCLUDED count expected 58, got $_excl_count"

  [ $_actual_count -eq 80 ] \
    && ok "guard: actual *.test.sh count=80" \
    || ko "guard: actual *.test.sh count expected 80, got $_actual_count"
