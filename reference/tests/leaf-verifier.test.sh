...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=13 EXCLUDED=36) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_allow_count" -eq 13 ] \
    && ok "guard: ALLOW count=13" \
    || ko "guard: ALLOW count expected 13, got $_allow_count"
  ...
  [ "$_actual_count" -eq 49 ] \
    && ok "guard: actual *.test.sh count=49" \
    || ko "guard: actual *.test.sh count expected 49, got $_actual_count"
