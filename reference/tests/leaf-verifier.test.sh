...
# =============================================================================
echo "=== Test 7: Composition/guard fail-closed (manifest completeness, ALLOW=16 EXCLUDED=40) ==="
...
if [ ! -f "$_MANIFEST" ]; then
  ...
  [ "$_actual_count" -eq 56 ] \
    && ok "guard: actual *.test.sh count=56" \
    || ko "guard: actual *.test.sh count expected 56, got $_actual_count"
