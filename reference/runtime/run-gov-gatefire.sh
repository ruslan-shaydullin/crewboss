...
cd /tmp/cbnet
PROXY_PID_FILE=/tmp/cbnet/proxy.pid
if [ -f "$PROXY_PID_FILE" ]; then kill "$(cat "$PROXY_PID_FILE")" 2>/dev/null; rm -f "$PROXY_PID_FILE"; fi
sleep 0.3; rm -f proxy.sock; : > proxy.log
nohup python3 /tmp/cbnet/proxy.py /tmp/cbnet/proxy.sock >/tmp/cbnet/proxy.out 2>&1 & PROXY=$!
echo "$PROXY" > "$PROXY_PID_FILE"
