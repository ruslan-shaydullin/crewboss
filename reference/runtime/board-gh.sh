...
[ -f "$LAUNCHABLE" ] || LAUNCHABLE="$HERE/../launcher/launchable.sh"
ensure_label(){ gh label create "$1" -R "$REPO" --color "${2:-ededed}" 2>/dev/null || true; }
iview(){
  local payload
  payload=$(gh issue view "$1" -R "$REPO" --json number,state,labels,body) || return 75
  printf '%s' "$payload" | jq -e 'type == "object" and (.labels | type == "array") and all(.labels[]; (.name|type)=="string") and ((.state|ascii_upcase)=="OPEN" or (.state|ascii_upcase)=="CLOSED")' >/dev/null || return 75
  printf '%s' "$payload"
}
ilist(){
  local payload
  payload=$(gh issue list "$@") || return 75
  printf '%s' "$payload" | jq -e 'type == "array" and all(.[]; type == "object" and (.labels | type == "array"))' >/dev/null || return 75
  printf '%s' "$payload"
}
...
case "$cmd" in
  launchable)
    ...
    ilist -R "$REPO" --state all --limit 100000 --json number,state,labels,body \
  ...
  plannable)  # charters awaiting decomposition: type:charter + status:needs-plan, open, not held
    ilist -R "$REPO" --state open --limit 100000 --json number,labels | jq -r '
  ...
  review-leaves)  # open agent leaves currently in status:review (awaiting integrator merge)
    ...
    ilist -R "$REPO" --state open --limit 100000 --json number,state,labels,body | jq -r \
  ...
  get)
    ...
    case "$field" in
      ...
      state)  echo "$j" | jq -r '
                if (.state|ascii_upcase)=="CLOSED" then "done"
                elif ([.labels[].name]|any(.=="hold" or .=="status:hold")) then "hold"
  ...
  claim)
    ...
    gh issue edit "$id" -R "$REPO" --add-label status:in-progress --add-label "claimed-by:$lid" --remove-label status:needs-rework >/dev/null
  ...
  route)
    ...
    case "$outcome" in
      review)  gh issue edit "$id" -R "$REPO" --remove-label status:in-progress --add-label status:review >/dev/null
               echo "#$id -> review" ;;
      blocked) gh issue edit "$id" -R "$REPO" --remove-label status:in-progress --remove-label status:review --add-label status:blocked >/dev/null
      ...
      needs-rework)
               ...
               nrlabs=$(iview "$id" | jq -r '.labels[].name | select(startswith("claimed-by:") or .=="status:in-progress" or .=="status:review" or .=="status:blocked")')
