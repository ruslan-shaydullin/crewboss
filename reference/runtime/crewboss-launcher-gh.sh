...
_finale_check_ci(){
  ...
  case "$new_state" in
    green)
      ...
      # AUTO merge (opt-in, #366 / per-charter label auto:merge): perform the final charter→main
      # human gate automatically once CI is green. Default off → charter PR waits for a human merge.
      _aml=$(gh issue view "$cid" -R "$CB_REPO" --json labels 2>/dev/null | jq -r '[.labels[].name]|index("auto:merge")!=null' 2>/dev/null || echo false)
      if [ "${CB_AUTO_MERGE:-0}" = "1" ] || [ "${_aml:-false}" = "true" ]; then
      ...
      for d in "$STATE"/*/; do [ -e "$d" ] || continue; id=$(basename "$d"); pid=$(sget "$id" pid)
        ...
        if [ "$(sget "$id" kind)" = "charter" ]; then
          ...
          if [ "$cst" = "plan-review" ] || [ "$cst" = "approved" ]; then
            ...
            _plag_json=$(gh issue view "$id" -R "$CB_REPO" --json labels 2>/dev/null || echo '{"labels":[]}')
            _plag=$(printf '%s' "$_plag_json" | jq -r '[.labels[].name]|index("plan:agreed")!=null' 2>/dev/null || echo false)
            _apa=$(printf '%s' "$_plag_json" | jq -r '[.labels[].name]|index("auto:plan-approve")!=null' 2>/dev/null || echo false)
            if [ -n "$_plrr" ] && [ "$cst" = "plan-review" ] && [ "$_plag" != "true" ]; then
              ...
              # AUTO plan-approval (opt-in, #366 / per-charter label auto:plan-approve): release leaves
              # automatically (no plan-convergence).
              if { [ "${CB_AUTO_PLAN_APPROVE:-0}" = "1" ] || [ "${_apa:-false}" = "true" ]; } && [ "$cst" = "plan-review" ] && [ -z "$_plrr" ]; then
