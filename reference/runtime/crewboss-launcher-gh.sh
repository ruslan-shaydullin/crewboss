...
_loop_is_alive(){   # args: running fresh → exit 0 = alive, exit 1 = idle
  ...
  if [ -n "${CB_MANIFEST:-}" ]; then
    ...
    _all_issues=$(gh issue list -R "$CB_REPO" --state all --limit 2000 \
...
_tqg_cycle(){
  ...
  _tqg_routing_leaves=$(gh issue list -R "$CB_REPO" --state open --limit 2000 \
  ...
  local _tqg_open
  _tqg_open=$(gh issue list -R "$CB_REPO" --state open --limit 2000 \
    --json number,labels,body 2>/dev/null | jq -r --argjson c "$cid" '
      def numsAfter($b; $k):
        [($b // "") | split("\n")[]
          | select(test("(?i)^[\\s*_>#-]*" + $k + "\\s*:"))]
  ...
  local _qa_leaf
  _qa_leaf=$(gh issue list -R "$CB_REPO" --state closed --limit 2000 \
    --json number,labels,body 2>/dev/null | jq -r --argjson c "$cid" '
      def numsAfter($b; $k):
        [($b // "") | split("\n")[]
          | select(test("(?i)^[\\s*_>#-]*" + $k + "\\s*:"))]
  ...
  local _impl_leaf
  _impl_leaf=$(gh issue list -R "$CB_REPO" --state closed --limit 2000 \
    --json number,labels,body 2>/dev/null | jq -r --argjson c "$cid" '
      def numsAfter($b; $k):
        [($b // "") | split("\n")[]
          | select(test("(?i)^[\\s*_>#-]*" + $k + "\\s*:"))]
...
_charter_finale_cycle(){
  ...
  local all_issues
  all_issues=$(gh issue list -R "$CB_REPO" --state all --limit 2000 \
...
_serializing_charter(){
  local _s
  _s=$(gh issue list -R "$CB_REPO" --state open --limit 2000 \
    ...
    while :; do
        ...
        if [ "$(sget "$id" kind)" = "plan-review" ]; then
          ...
          if [ "$_plok" = "true" ] || [ "$cst" = "needs-plan" ]; then
            ...
            if [ "$cst" = "needs-plan" ]; then
              _gc_snap=$(gh issue list -R "$CB_REPO" --state open --limit 2000 \
                         --json number,state,labels,body 2>/dev/null || echo "[]")
          ...
          if [ "$cst" = "plan-review" ] || [ "$cst" = "approved" ]; then
            ...
            if [ -n "$_plrr" ] && [ "$cst" = "plan-review" ] && [ "$_plag" != "true" ]; then
              ...
              if [ -n "$_rdl" ] && [ "$cst" = "plan-review" ]; then
                _leaf_count=$(gh issue list -R "$CB_REPO" --state all --limit 2000 \
      ...
      # claim new launchable up to the cap
        ...
        if [ -n "${CB_MANIFEST:-}" ]; then
          ...
          _analysis_boards=$(gh issue list -R "$CB_REPO" --state open --limit 2000 \
          ...
          if [ -n "$_approval_role" ] && [ -n "$_approval_threshold" ]; then
            _tr_charters=$(gh issue list -R "$CB_REPO" --state open --limit 2000 \
                           --json number,labels 2>/dev/null | jq -r '
              .[] | select([.labels[].name] | index("type:charter") != null)
            ...
            for cid in $_tr_charters; do
              ...
              if [ -z "$_comp_tsv" ]; then
                ...
                if [ "$_fround" -ge "$_fcap" ]; then
                  _all_hd=$(gh issue list -R "$CB_REPO" --state open --limit 2000 --json number,labels,body 2>/dev/null || echo "[]")
...
Charter: #$cid" \
              ...
              if [ "$_escalate" = "1" ]; then
                ...
                _all_hd=$(gh issue list -R "$CB_REPO" --state open --limit 2000 \
...
Charter: #$cid"
        ...
        _conflict_boards=$(gh issue list -R "$CB_REPO" --state open --limit 2000 \
                           --json number,labels 2>/dev/null | jq -r '
          .[] | select([.labels[].name] | index("type:charter") != null)
        ...
        if [ -n "$_plan_review_role" ]; then
          _plr_charters=$(gh issue list -R "$CB_REPO" --state open --limit 2000 --json number,labels 2>/dev/null | jq -r '
            .[] | select([.labels[].name] | index("type:charter") != null)
...
Charter: #$cid" \
        ...
        if [ -n "$_plan_review_role" ]; then
          _limbo_charters=$(gh issue list -R "$CB_REPO" --state open --limit 2000 --json number,labels 2>/dev/null | jq -r '
            .[] | select([.labels[].name] | index("type:charter") != null)
        ...
        _acc_review_role=$(manifest_policy "${CB_MANIFEST:-}" acceptance_review_role 2>/dev/null || true)
        if [ -n "$_acc_review_role" ]; then
          _acr_charters=$(gh issue list -R "$CB_REPO" --state open --limit 2000 --json number,labels 2>/dev/null | jq -r '
            .[] | select([.labels[].name] | index("type:charter") != null)
